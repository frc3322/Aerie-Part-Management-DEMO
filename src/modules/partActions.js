...
function buildAppendedNotes(notes, location) {
  const storageText =
    location && location.trim() !== "" ? location.trim() : "[unspecified]";
  const suffix = `\nPart stored in ${storageText} upon completion -autogenerated-`;
  return `${notes || ""}${suffix}`;
}
...
async function handleStorageSubmit(event) {
  ...
  const newNotes = buildAppendedNotes(part.notes, location);
  try {
    setStorageModalLoading(true);
    if (triggerButton) triggerButton.disabled = true;
    const completedPart = await apiCompletePart(part.id);
    const updatedPart = await apiUpdatePart(part.id, { notes: newNotes });
    const mergedPart = {
      ...completedPart,
      ...updatedPart,
      notes: updatedPart.notes || newNotes,
    };
    updatePartInState(part.id, mergedPart);
...
export async function approvePart(index, event) {
  ...
  try {
    ...
    const updatedPart = await apiApprovePart(part.id);
    updatePartInState(part.id, updatedPart);
